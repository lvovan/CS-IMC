...
0. Si possible, installez [Azure Data Studio](https://docs.microsoft.com/fr-fr/sql/azure-data-studio/download-azure-data-studio?view=sql-server-ver15) pour faciliter la création de vos requêtes SQL. Si ce n'est pas possible, vous pourrez également requêter depuis votre navigateur.
1. Connectez-vous au portail Azure, et ouvrez la page correspondant la base de données SQL `tpbdd-movies-sql`
...
# Neo4j
Neo4j est une base de données graphe. Les données sont représentées par des nœuds, des relations entre les noeuds et des propriétés:
1. Les nœuds et les relations contiennent des propriétés
2. Les relations connectent les nœuds
3. Les propriétés sont des paires clé-valeur
...
Cypher est basé sur des **patterns**.

Un pattern décrit les données et interroge le graphe à l'aide d'une forme visuelle très similaire à la façon dont sont généralement illustrées sur papier les données dans un graphe.
On utilise des:
* Cercles `()` pour représneter les **nœuds**
* Flèches `-->` pour représenter les **relations**

**Patterns** :
1. nœud a : `(a)`
2. une relation entre deux noeuds : `(a)–>(b)` 
3. un chemin : `(a)–>(b)<–(c)`
4. un label (ou étiquette) : `(a:label)`

Les patterns sont utilisés pour le requêtage et visent à surtout réprésenter la structure des liens entre les noeuds, comme on peut le voir dans la liste suivante:
1. `(a)–(b)`
2. `(a)-[r]- >(b) `
3. `(a)-[r:REL_TYPE]->(b) `
4. `(a)-[:REL_TYPE]->(b) `
...
**Exercise 1**: Ajoutez une personne ayant votre prénom et votre nom dans le graphe. Verifiez qui le noeud a bien éte crée. 

**Exercise 2**: Ajoutez un film nommé `L'histoire de mon 20 au cours Infrastructure de donnees`

**Exercise 3**: Ajoutez la relation `ACTED_IN` qui modélise votre participation à ce film en tant qu'acteur/actrice

**Exercise 4**: Ajoutez deux de vos professeurs/enseignants comme réalisateurs/réalisatrices de ce film.

**Exercise 5**: Affichez le noeud représentant l'acteur nommé `Jude Law`, et visualisez son année de naissance.

**Exercise 6**: Visualisez l'ensemble des films.

**Exercise 7**: Trouvez les noms des artistes nés en `1960`, affichez ensuite leur nombre.

**Exercise 8**: Trouver l'ensemble des acteurs (sans entrées doublons) qui ont joué dans plus d'un film.

**Exercise 9**: Trouvez les artistes ayant eu plusieurs responsabilités au cours de leur carrière (acteur, directeur, producteur...).

**Exercise 10**: Montrez les artistes ayant eu plusieurs responsabilités dans un même film (ex: à la fois acteur et directeur, ou toute autre combinaison) et les titres de ces films.

**Exercise 11**: Trouver le nom du ou des film(s) ayant le plus d'acteurs.
